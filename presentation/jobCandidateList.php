 		...
 		public function __construct(){
 			if(isset($_GET['job_application'])||isset($_GET['vipanan_view_company_job_applicant_list'])||isset($_GET['job_applicants'])){
 				...
 				foreach ($allCandidateList as $candidateDetail) {
					 ...
					 $graduation_Details = $candidateDetail['graduation_details'];

					 $grad_details = explode(',',$graduation_Details);
 					$candidateDetail['graduation_details'] = array('collegeName' => $grad_details[0], 'course' => $grad_details[1],'Specilization' => $grad_details[2],'Year_of_Complition' => $grad_details[3],'percentage' => $grad_details[4]);
 				$candidateDetail['post_graduation_details']= array('collegeName' => $post_grad_details[0], 'course' => $post_grad_details[1],'Specilization' => $post_grad_details[2],'Year_of_Complition' => $post_grad_details[3],'percentage' => $post_grad_details[4]);
