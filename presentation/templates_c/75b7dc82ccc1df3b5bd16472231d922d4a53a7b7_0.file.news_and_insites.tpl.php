<?php
/* Smarty version 3.1.33, created on 2019-07-20 06:57:55
  from 'C:\xampp\htdocs\rbs\presentation\templates\news_and_insites.tpl' */

/* @var Smarty_Internal_Template $_smarty_tpl */
if ($_smarty_tpl->_decodeProperties($_smarty_tpl, array (
  'version' => '3.1.33',
  'unifunc' => 'content_5d32bb7331f001_54147650',
  'has_nocache_code' => false,
  'file_dependency' => 
  array (
    '75b7dc82ccc1df3b5bd16472231d922d4a53a7b7' => 
    array (
      0 => 'C:\\xampp\\htdocs\\rbs\\presentation\\templates\\news_and_insites.tpl',
      1 => 1563605874,
      2 => 'file',
    ),
  ),
  'includes' => 
  array (
    'file:header.tpl' => 1,
    'file:footer.tpl' => 1,
  ),
),false)) {
function content_5d32bb7331f001_54147650 (Smarty_Internal_Template $_smarty_tpl) {
?><!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <link
      rel="stylesheet"
      href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css"
      integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T"
      crossorigin="anonymous"
    />
    <link rel="stylesheet" type="text/css" href="styles/slick.css" />
    <link rel="stylesheet" type="text/css" href="styles/slick-theme.css" />
    <link rel="stylesheet" type="text/css" href="styles/newsinsites.css" />

    <title>Home|WIT|News & Insites</title>
  </head>
  <body>
    <?php $_smarty_tpl->_subTemplateRender('file:header.tpl', $_smarty_tpl->cache_id, $_smarty_tpl->compile_id, 0, $_smarty_tpl->cache_lifetime, array(), 0, false);
?>
    <div class="bgimg">
      <div class="container-fluid p-5">
        <div class="container">
          <div class="row">
            <div class="col-md-12">
              <p
                class="text-center subHeading"
                style="color: white !important;"
              >
                WiT IN MEDIA
              </p>
              <div class="responsive">
                <div class="slide p-2">
                  <div class="card">
                    <div class="img-div-fix-news-insites">
                      <img
                        class="card-img-top img-fluid"
                        src="images/wit-in-indian-times.png"
                        alt="Card image"
                      />
                    </div>
                    <div class="card-body">
                      <h6 class="card-title  titletxtBound">
                        Mastercard Leads The Launch Of The WomeninTech Forum In
                        Pune
                      </h6>
                      <p class="card-text txtBound text-justify">
                        The inaugural event, hosted by Mastercard at the
                        Westin,Pune, witnessed participation from leading
                        technology...
                      </p>
                      <a href="?witInMedia=true" class="btn witBtn btn-block"
                        >FIND MORE</a
                      >
                    </div>
                  </div>
                </div>
                <div class="slide p-2">
                  <div class="card">
                    <div class="img-div-fix-news-insites">
                      <img
                        class="card-img-top img-fluid"
                        src="images/wit-in-indian-express.png"
                        alt="Card image"
                      />
                    </div>
                    <div class="card-body">
                      <h6 class="card-title  titletxtBound">
                        New 'WomenInTech' Forum Launched To Promote STEM Careers
                        Among Girls
                      </h6>
                      <p class="card-text txtBound text-justify">
                        Royal Bank of Scotland (RBS), India kicked start a
                        collaborative industry initiative to address the
                        sharp...
                      </p>
                      <a href="?witInMedia=true" class="btn witBtn btn-block"
                        >FIND MORE</a
                      >
                    </div>
                  </div>
                </div>
                <div class="slide p-2">
                  <div class="card">
                    <div class="img-div-fix-news-insites">
                      <img
                        class="card-img-top img-fluid"
                        src="images/wit-in-media-united-news.png"
                        alt="Card image"
                      />
                    </div>
                    <div class="card-body">
                      <h6 class="card-title  titletxtBound">
                        RBS India Facilitates Setting-Up Of WomenInTech Forum
                      </h6>
                      <p class="card-text txtBound text-justify">
                        RBS India in association with prominent technology
                        corporates, Academia and government participation has...
                      </p>
                      <a href="?witInMedia=true" class="btn witBtn btn-block"
                        >FIND MORE</a
                      >
                    </div>
                  </div>
                </div>
                <div class="slide p-2">
                  <div class="card">
                    <div class="img-div-fix-news-insites">
                      <img
                        class="card-img-top img-fluid"
                        src="images/wit-in-media.png"
                        alt="Card image"
                      />
                    </div>
                    <div class="card-body">
                      <h6 class="card-title  titletxtBound">
                        WiT Forum To Work On Enhancing Womens Participation In
                        STEM
                      </h6>
                      <p class="card-text txtBound text-justify">
                        Leading corporates and academia have joined hands to set
                        up 'WomenInTech (WiT) Forum that will work towards
                        encouraging...
                      </p>
                      <a href="?witInMedia=true" class="btn witBtn btn-block"
                        >FIND MORE</a
                      >
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col-sm-5"></div>
            <div class="col-sm-2 text-center">
              <a href="?witInMedia=true" class="btn  witBtn px-3">FIND MORE</a>
            </div>
            <div class="col-sm-5"></div>
          </div>
        </div>
      </div>
    </div>
    <div class="container">
      <div class="row mt-5 ">
        <div class="col-sm-12 sjd">
          <h1 class="text-center subHeading">OUR BLOGS</h1>
          <div class="row p-3">
            <div class="col-sm-3 p-2">
              <div class="card newsForDiv" style="width: 16rem;">
                <div class="img-div fix">
                  <img
                    class="card-img-top"
                    src="images/blog.jpg"
                    alt="Card image cap"
                  />
                </div>
                <div class="card-body">
                  <h6 class="card-title  titletxtBoundBlog">
                    Machine Learning & Humanizing Big Data
                  </h6>
                  <p class="card-text txtBoundBlog">
                    It won’t be long before machine learning becomes a kind of
                    “new normal,” with people expecting this type of Artificial
                    Intelligence (AI) as a component in every form of
                    technology.
                  </p>
                </div>
                <div class="card-body">
                  <a href="?blogs=true" class="card-link stretched-link"></a>
                </div>
              </div>
            </div>
            <div class="col-sm-3 p-2">
              <div class="card newsForDiv" style="width: 16rem;">
                <div class="img-div fix">
                  <img
                    class="card-img-top"
                    src="images/blog.jpg"
                    alt="Card image cap"
                  />
                </div>
                <div class="card-body">
                  <h6 class="card-title  titletxtBoundBlog">
                    WIT to Hold Leadership Awards Round Table for 2019 Planning
                  </h6>
                  <p class="card-text txtBoundBlog">
                    Women in Technology will hold a private Round Table and
                    networking event with the winners and finalists from our
                    upcoming Leadership Awards event May 19th. "These are the
                    best women we
                  </p>
                </div>
                <div class="card-body">
                  <a href="?blogs=true" class="card-link stretched-link"></a>
                </div>
              </div>
            </div>
            <div class="col-sm-3 p-2">
              <div class="card newsForDiv" style="width: 16rem;">
                <div class="img-div fix">
                  <img
                    class="card-img-top"
                    src="images/blog.jpg"
                    alt="Card image cap"
                  />
                </div>
                <div class="card-body">
                  <h6 class="card-title  titletxtBoundBlog">
                    Marketing Implications of Augmented Reality & Virtual
                    Reality
                  </h6>
                  <p class="card-text txtBoundBlog">
                    Worldwide revenues for the augmented reality and virtual
                    reality (AR/VR) market are forecast to increase by 100% or
                    more over each of the next four years, according to the
                    latest update from the
                  </p>
                </div>
                <div class="card-body">
                  <a href="?blogs=true" class="card-link stretched-link"></a>
                </div>
              </div>
            </div>
            <div class="col-sm-3 p-2">
              <div class="card newsForDiv" style="width: 16rem;">
                <div class="img-div fix">
                  <img
                    class="card-img-top"
                    src="images/blog.jpg"
                    alt="Card image cap"
                  />
                </div>
                <div class="card-body">
                  <h6 class="card-title  titletxtBoundBlog">
                    Does WIT have Something for Me?
                  </h6>
                  <p class="card-text txtBoundBlog">
                    Since the Women in Technology (WIT) calendar runs from July
                    1 – June 30, our program year is winding down and planning
                    has already started for next year, including the recruitment
                    of next
                  </p>
                </div>
                <div class="card-body">
                  <a href="?blogs=true" class="card-link stretched-link"></a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col-sm-5"></div>
        <div class="col-sm-2 text-center">
          <a
            href="
            ?blogs=true"
            class="btn witBtn px-3"
            >FIND MORE</a
          >
        </div>
        <div class="col-sm-5"></div>
      </div>
    </div>
    <div class="bgimgg mt-5">
      <div class="container-fluid p-5">
        <div class="container">
          <div class="row">
            <div class="col-md-12">
              <h1
                class="text-center subHeading text-white"
                style="color:white !important;"
              >
                WiT SPONSORS
              </h1>
              <div class="responsive">
                <div class="slide p-2">
                  <div class="card">
                    <div class="img-div-fix-sponsor p-2">
                      <img
                        class="card-img-top img-fluid"
                        src="images/ibm.png"
                        alt="Card image"
                        style="width:100%"
                      />
                    </div>
                  </div>
                </div>
                <div class="slide p-2">
                  <div class="card">
                    <div class="img-div-fix-sponsor p-2">
                      <img
                        class="card-img-top img-fluid"
                        src="images/jcpenney.png"
                        alt="Card image"
                        style="width:100%"
                      />
                    </div>
                  </div>
                </div>
                <div class="slide p-2">
                  <div class="card">
                    <div class="img-div-fix-sponsor p-2">
                      <img
                        class="card-img-top img-fluid"
                        src="images/sapient.png"
                        alt="Card image"
                        style="width:100%"
                      />
                    </div>
                  </div>
                </div>
                <div class="slide p-2">
                  <div class="card">
                    <div class="img-div-fix-sponsor p-2">
                      <img
                        class="card-img-top img-fluid"
                        src="images/RBOS-2.png"
                        alt="Card image"
                        style="width:100%"
                      />
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col-sm-5"></div>
            <div class="col-sm-2  text-center">
              <a href="?witSponsors=true" class="btn witBtn px-3">FIND MORE</a>
            </div>
            <div class="col-sm-5"></div>
          </div>
        </div>
      </div>
    </div>

    <div class="mt-5">
      <div class="container-fluid p-5 didYouKnowContainer">
        <div class="container">
          <div class="row">
            <div class="col-md-2"></div>
            <div class="col-md-8">
              <h1 class="text-center subHeading ">DID YOU KNOW</h1>
              <hr />
              <div class="responsive2">
                <div class="slide p-4">
                  <div class="card chaya ">
                    <div class="card-body">
                      <p class=" feedbacktext  diduknowBound">
                        <i class="fa fa-quote-left left"></i> Women make up
                        <b>40%</b> of the world’s workforce, according to the
                        <b>World Bank.</b> Yet, the technology field is
                        notoriously male-dominated at all levels – Considerably
                        worse than in non-technology industries <br />
                        – As reported by Payscale
                        <i class="fa fa-quote-right right"></i>
                      </p>
                    </div>
                  </div>
                </div>
                <div class="slide p-4">
                  <div class="card chaya">
                    <div class="card-body">
                      <p class=" feedbacktext  diduknowBound">
                        <i class="fa fa-quote-left left"></i> Only <b>21%</b> of
                        <b>executives in tech</b> are women – This despite
                        evidence that more women lead to greater
                        <b>innovation</b> and enhanced <b>profitability</b>
                      </p>
                    </div>
                  </div>
                </div>
                <div class="slide p-4">
                  <div class="card chaya">
                    <div class="card-body">
                      <p class=" feedbacktext  diduknowBound">
                        <i class="fa fa-quote-left left"></i> The two biggest
                        barriers women say they face in the technology workplace
                        are lack of mentors <b> (48%)</b> and lack of female
                        role models <b>(42%)</b> ,gender bias at workplace,
                        unequal growth opportunities and unequal pay for same
                        <b>skills</b> <i class="fa fa-quote-right right"></i>
                      </p>
                    </div>
                  </div>
                </div>
                <div class="slide p-4">
                  <div class="card chaya ">
                    <div class="card-body">
                      <p class="feedbacktext  diduknowBound">
                        <i class="fa fa-quote-left left"></i>
                        <b> 93% girls</b> surveyed in India between
                        <b>12 to 14-years</b> consider Science, Technology,
                        Engineering and Mathematics (STEM) related careers early
                        on. However, <b>38% Indian</b> teenage girls believe
                        that they are <b>‘less likely’</b> to take up STEM
                        because the industry is <b>‘male-dominated’</b> and many
                        others drop out before reaching leadership levels
                        <i class="fa fa-quote-right right"></i>
                      </p>
                    </div>
                  </div>
                </div>
                <div class="slide p-4">
                  <div class="card chaya ">
                    <div class="card-body">
                      <p class="feedbacktext  diduknowBound">
                        <i class="fa fa-quote-left left"></i> In the
                        <b>2016–17,</b> colleges approved by the
                        <b>All India Council for Technical Education (AICTE)</b>
                        enrolled <b>717,506</b> undergraduate students –
                        <b>33.7%</b> were female. Women represented
                        <b>28.1%</b> of participants in Bachelor of Engineering
                        and Bachelor of Technology degree (Research 2018; India
                        Under Graduate Education, data as per
                        <b>MHRD</b> 2016-2017)
                        <i class="fa fa-quote-right right"></i>
                      </p>
                    </div>
                  </div>
                </div>
                <div class="slide p-4">
                  <div class="card chaya ">
                    <div class="card-body">
                      <p class="feedbacktext  diduknowBound">
                        <i class="fa fa-quote-left left"></i> Indian technology
                        industry has <b>26%</b> women in
                        <b>engineering</b> roles
                        <i class="fa fa-quote-right right"></i>
                      </p>
                    </div>
                  </div>
                </div>
                <div class="slide p-4">
                  <div class="card chaya ">
                    <div class="card-body">
                      <p class="feedbacktext  diduknowBound">
                        <i class="fa fa-quote-left left"></i> Nearly
                        <b>50%</b> women <b>engineers</b> quit technology
                        <i class="fa fa-quote-right right"></i>
                      </p>
                    </div>
                  </div>
                </div>
                <div class="slide p-4">
                  <div class="card chaya ">
                    <div class="card-body">
                      <p class="feedbacktext  diduknowBound">
                        <i class="fa fa-quote-left left"></i> Only
                        <b>7%</b> women reach the <b>C-Suite</b> Source:
                        <b> Nasscom Report 2017</b>
                        <i class="fa fa-quote-right right"></i>
                      </p>
                    </div>
                  </div>
                </div>
                <div class="slide p-4">
                  <div class="card chaya ">
                    <div class="card-body">
                      <p class="feedbacktext  diduknowBound">
                        <i class="fa fa-quote-left left"></i>
                        <b>1.5 million</b> IT engineers pass out each year in
                        India through over <b>3400+</b> engineering colleges
                        peppered across the country. Statistics place employable
                        numbers from these graduates at a mere <b>7%</b>.
                        <i class="fa fa-quote-right right"></i>
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-2"></div>
          </div>
        </div>
      </div>
    </div>
    <?php $_smarty_tpl->_subTemplateRender('file:footer.tpl', $_smarty_tpl->cache_id, $_smarty_tpl->compile_id, 0, $_smarty_tpl->cache_lifetime, array(), 0, false);
?>

    <?php echo '<script'; ?>

      src="https://code.jquery.com/jquery-3.3.1.slim.min.js"
      integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo"
      crossorigin="anonymous"
    ><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>

      src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"
      integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1"
      crossorigin="anonymous"
    ><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>

      src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"
      integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM"
      crossorigin="anonymous"
    ><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>

      type="text/javascript"
      src="//code.jquery.com/jquery-1.11.0.min.js"
    ><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>

      type="text/javascript"
      src="//code.jquery.com/jquery-migrate-1.2.1.min.js"
    ><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 type="text/javascript" src="js/slick.min.js"><?php echo '</script'; ?>
>
    <?php echo '<script'; ?>
 src="js/newsinsitesslick.js"><?php echo '</script'; ?>
>
  </body>
</html>
<?php }
}
